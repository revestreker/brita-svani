...
// ─── PAN HINT ────────────────────────────────────────────────────────────────
// Called from loadScene in the full engine build
function showPanHintIfNeeded(scene) {
  const panHint = document.getElementById("pan-hint");
  if (!panHint) return;
  if (scene.type === "pan" && scene.showPanHint) {
    panHint.style.display = "flex";
    panHint.style.opacity = "1";
    setTimeout(() => {
      panHint.style.transition = "opacity 0.8s ease";
      panHint.style.opacity = "0";
      setTimeout(() => {
        panHint.style.display = "none";
        panHint.style.opacity = "1";
        panHint.style.transition = "";
      }, 800);
    }, 3000);
  } else {
    panHint.style.display = "none";
